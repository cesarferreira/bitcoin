Cesarcoin Core version 0.11.0 is now available from:

  <https://cesarcoin.org/bin/cesarcoin-core-0.11.0/>
...
For example, add the following to `cesarcoin.conf`:
...
related to the blockchain in the cesarcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Cesarcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Cesarcoin Core will 
...
For an example script see [cesarcoin-submittx](https://github.com/laanwj/cesarcoin-submittx).
...
- #5334 `cf87536` libcesarcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new cesarcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only CesarcoinAmountField
- #5753 `0f44672` Add cesarcoin logo to about screen
...
- #5809 `46bfbe7` Add cesarcoin-cli man page
...
- #5849 `41259ca` contrib/init/cesarcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Cesarcoin-Qt.app to Cesarcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for cesarcoin-tx)
