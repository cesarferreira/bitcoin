#!/bin/bash
# Copyright (c) 2013 The Cesarcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Cesarcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Cesarcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Cesarcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Cesarcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Cesarcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Cesarcoin 0.8 datadir."
    ;;
esac
