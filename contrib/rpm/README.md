...
The RPM spec file provided here is for Cesarcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://cesarcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Cesarcoin git repository](https://github.com/cesarcoin/cesarcoin) but are not part of
...
Currently only `Source10` is used, for the example `cesarcoin.conf` file.
...
in the cesarcoin git repository. At present only an SVG version of the Cesarcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cesarcoin
...
needed when 0.12.1 is released, a proper fix is already in the Cesarcoin git
master branch.
...
idea to build Cesarcoin Core against a static version of that library compiled
...
The `cesarcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `cesarcoin-server` package. At this point, most RPM based
...
The `cesarcoin-server` package should have SELinux support. How to properly do
...
Cesarcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cesarcoin/). He has been
packaging Cesarcoin for Fedora at least since 2012.
...
4. I renamed the `cesarcoin` package that contains the Qt GUI to `cesarcoin-core` as
that appears to be how the general population refers to it, in contrast to
`cesarcoin-xt` or `cesarcoin-classic`. I wanted to make sure the general population
