...
###   cesarcoincore.org and cesarcoin.org and compares them.
...
WORKINGDIR="/tmp/cesarcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://cesarcoincore.org"
HOST2="https://cesarcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="cesarcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'cesarcoin-',
...
if [ $? -ne 0 ]; then
   echo "cesarcoin.org failed to provide signature file, but cesarcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "cesarcoin.org and cesarcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Cesarcoin Core binary release signing key installed?"
   fi
